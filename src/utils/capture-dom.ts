...
export async function captureDOMStructure(
  ...
  const domData = await page.evaluate(() => {
    ...
    function createConciseTree() {
          ...
          return (
            (li.querySelector('span[data-gs][aria-label*="US dollars"]') !==
              ...
              (li.textContent?.includes("$") &&
                Array.from(li.querySelectorAll("div")).some((div) =>
                  /^\d+\s*hr/.test(div.textContent?.trim() || ""),
                ))) &&
      ...
      const priceElements = Array.from(
        document.querySelectorAll('span[aria-label*="US dollars"], [data-gs]'),
      );
      const airlineElements = Array.from(
        document.querySelectorAll(
          'div[aria-label*="Airlines"], img[alt*="Airlines"]',
        ),
      );
      const airportElements = Array.from(
        document.querySelectorAll('span[aria-label*="Airport"]'),
      );
      const timeElements = Array.from(document.querySelectorAll("div")).filter(
        (el) =>
      ...
      flightElements.slice(0, 5).forEach((el, i) => {
        ...
        const ariaLabels = ariaElements
          .map((ae) => ae.getAttribute("aria-label"))
          .filter(Boolean);
  ...
  const outputFilePath = path.join(
    outputDir,
    `${prefix}-dom-structure-${timestamp}.txt`,
  );
